--Task(1) Write a query to calculate the total sales and average sales for each product category from the Sales table.
...
SELECT ProductID,sum(SaleAmount) As Total_sales ,AVG(SaleAmount) AS Average_Sales FROM Sales 
Group by ProductID;
...
--Task(3) Write a query to count the number of rows in the Customers table using COUNT(*).
...
--Task(4) Write a query to count the number of unique product categories (COUNT(DISTINCT Category)) from the Products table.
...
SELECT COUNT(DISTINCT Category) AS U_PRODUCTS_CATEGORY FROM Products;
select*from products

--Task(5) Write a query to find the total (SUM) sales amount for the product with id 7 in the Sales table.
SELECT ProductID,SUM(SaleAmount) AS TOTAL_SALES_ID7 FROM Sales
WHERE ProductID =7
GROUP BY ProductID;


--Task(6) Write a query to calculate the average (AVG) age of employees in the Employees table.
SELECT AVG(AGE) AS AVERAGE_AGE FROM Employees;


--Task(7) Write a query that uses GROUP BY to count the number of employees in each department.
select DepartmentName,Count(EmployeeID) AS EMP_NUMBERS from Employees
group by DepartmentName


--Task(8) Write a query to show the minimum and maximum Price of products grouped by Category. Use products table.
SELECT Category,Max(price) Max_Price_Products,Min(price) as MIN_Price_Products from Products
Group By Category;


--Task(9) Write a query to calculate the total (SUM) sales per Customer in the Sales table.
SELECT CustomerID,SUM(SaleAmount) AS Sales_Per_Customer FROM Sales
GROUP BY CustomerID;


--Task(10) Write a query to use HAVING to filter departments having more than 5 employees from the Employees table.(DeptID is enough, if you don't have DeptName).
SELECT DepartmentName,count(EmployeeID) as Emp_numbers 
FROM Employees
GROUP BY DepartmentName
HAVING COUNT(EmployeeID)>5;

--Task(11) Write a query to calculate the total sales and average sales for each product category from the Sales table.
SELECT ProductID,SUM(SaleAmount) AS TOTAL_SALES_PER_PRODUCT,avg(SaleAmount) AS AVERAGE_SALES_PER_PRODUCT FROM Sales
GROUP BY ProductID;



--Task(12) Write a query that uses COUNT(columnname) to count the number of employees from the Department HR.
SELECT DepartmentName,count(EmployeeID) AS HR_EMP FROM Employees
WHERE DepartmentName='HR'
GROUP BY DepartmentName;

--Task(13) Write a query that finds the highest (MAX) and lowest (MIN) Salary by department in the Employees table.(DeptID is enough, if you don't have DeptName).
SELECT DepartmentName,MAX(Salary) as Max_salary_per_Department,MIN(Salary) as MIN_salary_per_Department
FROM Employees
GROUP BY DepartmentName;


--Task(14) Write a query that uses GROUP BY to calculate the average salary per Department.(DeptID is enough, if you don't have DeptName).
SELECT DepartmentName,AVG(Salary) AS AVERAGE_SALARY_DEPARTMENTS
FROM Employees
GROUP BY DepartmentName;
...
SELECT DepartmentName,AVG(Salary) as AVG_SAL_PerDEP,Count(*) AS EMP_EACH_DEP
FROM Employees
GROUP BY DepartmentName;

--Task(16) Write a query that uses HAVING to filter product categories with an average price greater than 400.
SELECT Category,AVG(Price) as AVG_pricr_prod
FROM Products
GROUP BY Category
